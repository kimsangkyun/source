package sdd.thirdparty.fullscroll;
...
import sdd.interiorexplorers.MapView;
 ...
 * takes care of its own scrolling. Most importantly, doing this defeats all of
 * the important optimizations in ListView for dealing with large lists, since
 * it effectively forces the ListView to display its entire list of items to
 ...
 * 
 * <p>
 * FullScrollView supports vertical and horizontal scrolling. FullScrollView is
 * based on ScrollView and extended according to symmetry so horizontal
 * scrolling would be possible. Contains many bugs and documentation has not
 * been updated, but kinda works when containing one TextView as a child. pjv
 * 2009-01-24
 */
public class FullScrollView extends FrameLayout {
	...
	public FullScrollView(Context context, AttributeSet attrs, int defStyle) {
		...
		// net.lp.androidsfortune.R.attr.ScrollView, defStyle, 0);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (child.getMeasuredWidth() < width
				...
				&& widthMode != MeasureSpec.UNSPECIFIED && mFillViewportX
				&& mFillViewportY) {
		...
		} else if (child.getMeasuredHeight() < height
				&& heightMode != MeasureSpec.UNSPECIFIED && mFillViewportY) {
			...
			child.measure(childWidthMeasureSpec, childHeightMeasureSpec);
		} else if (child.getMeasuredWidth() < width
				&& widthMode != MeasureSpec.UNSPECIFIED && mFillViewportX) {
			final FrameLayout.LayoutParams lp = (LayoutParams) child
					.getLayoutParams();
	...
	public boolean dispatchKeyEvent(KeyEvent event) {
		...
		if (mapView != null) {
			mapView.keyPressed(event.getKeyCode());
		}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if ((action == MotionEvent.ACTION_MOVE)
				&& (mIsBeingDraggedX || mIsBeingDraggedY)) {
			return true;
		}
		...
		case MotionEvent.ACTION_MOVE:
			...
			/*
			 * Locally do absolute value. mLastMotionY is set to the x value of
			 * the down event.
			 */
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (localLOGV)
				Log.v(TAG, "onTouchEvent ActionMove deltaX=" + deltaX
						+ " deltaY=" + deltaY);
		...
		case MotionEvent.ACTION_UP:
			...
			if ((Math.abs(initialVelocityX) > ViewConfiguration
					.getMinimumFlingVelocity())
					&& (getChildCount() > 0)) {
				flingX(-initialVelocityX);
			}

			if ((Math.abs(initialVelocityY) > ViewConfiguration
					.getMinimumFlingVelocity())
					&& (getChildCount() > 0)) {
	 ...
	 *            leftFocus is true, or at the bottom of the bounds if leftFocus
	 *            is false
	 * @param left
	 ...
	 *         found
	 */
	private View findFocusableViewInBoundsX(boolean leftFocus, int left,
			int right) {
		...
		/*
		 * A fully contained focusable is one where its left is right of the
		 * bound's left, and its right is left of the bound's right. A partially
	 ...
	 *         found
	 */
	private View findFocusableViewInBoundsY(boolean topFocus, int top,
			int bottom) {
	...
	/**
	 * Handle scrolling in response to an up or down or left or right arrow
	 * click.
	 * 
	 * @param direction
	...
	public boolean arrowScroll(int direction) {
		...
		if (nextFocused != null
				&& isWithinDeltaOfScreenX(nextFocused, maxJumpX)
				&& isWithinDeltaOfScreenY(nextFocused, maxJumpY)) {// TODO
			...
			if (scrollDeltaX == 0 && scrollDeltaY == 0) {
				...
				doScrollY(direction == View.FOCUS_DOWN ? scrollDeltaY
						: -scrollDeltaY);
			} else if (scrollDeltaY == 0) {
				doScrollX(direction == View.FOCUS_DOWN ? scrollDeltaX
						: -scrollDeltaX);
			}
		}
	...
	private boolean isOffScreen(View descendant) {
		return !isWithinDeltaOfScreenX(descendant, 0)
				&& !isWithinDeltaOfScreenY(descendant, 0);
	}
	...
	public final void smoothScrollBy(int dx, int dy) {
		...
		if (duration > ANIMATED_SCROLL_GAP) {
			if (localLOGV)
				Log.v(TAG, "Smooth scroll: mScrollX=" + mScrollX + " dx=" + dx
						+ "mScrollY=" + mScrollY + " dy=" + dy);
			...
			if (localLOGV)
				Log.v(TAG, "Immediate scroll: mScrollX=" + mScrollX + " dx="
						+ dx + "mScrollY=" + mScrollY + " dy=" + dy);
			scrollBy(dx, dy);
		}
	...
	public void computeScroll() {
		if (mScroller.computeScrollOffset()) {
			...
			if (getChildCount() > 0) {
				...
				if (localLOGV)
					Log.v(TAG, "mScrollX=" + mScrollX + " x=" + x + " width="
							+ this.getWidth() + " child width="
							+ child.getWidth() + " mScrollY=" + mScrollY
	...
	protected int computeScrollDeltaToGetChildRectOnScreenX(Rect rect) {
		...
		if (localLOGV)
			Log.v(TAG, "child=" + rect.toString() + " screenLeft=" + screenLeft
					+ " screenRight=" + screenRight + " width=" + width);
		if (rect.right > screenRight && rect.left > screenLeft) {
			...
			if (localLOGV)
				Log.v(TAG, "scrollXDelta=" + scrollXDelta + " distanceToRight="
						+ distanceToRight);
			scrollXDelta = Math.min(scrollXDelta, distanceToRight);

		} else if (rect.left < screenLeft && rect.right < screenRight) {
			// need to move left to get it in view: move left just enough so
			// that
	...
	protected int computeScrollDeltaToGetChildRectOnScreenY(Rect rect) {
		...
		if (localLOGV)
			Log.v(TAG, "child=" + rect.toString() + " screenTop=" + screenTop
					+ " screenBottom=" + screenBottom + " height=" + height);
		if (rect.bottom > screenBottom && rect.top > screenTop) {
			...
			if (localLOGV)
				Log.v(TAG, "scrollYDelta=" + scrollYDelta
	...
	public void flingX(int velocityX) {
		...
		mScroller.fling(mScrollX, mScrollY, velocityX, 0, 0, 0, 0, right
				- width);

		final boolean movingRight = velocityX > 0;

		View newFocused = findFocusableViewInMyBoundsX(movingRight, mScroller
				.getFinalX(), findFocus());
		if (newFocused == null) {
			newFocused = this;
		}
		...
		if (newFocused != findFocus()
				&& newFocused.requestFocus(movingRight ? View.FOCUS_DOWN
						: View.FOCUS_UP)) {
			mScrollViewMovedFocus = true;
			mScrollViewMovedFocus = false;
		}
	...
	public void flingY(int velocityY) {
		...
		mScroller.fling(mScrollX, mScrollY, 0, velocityY, 0, 0, 0, bottom
				- height);

		final boolean movingDown = velocityY > 0;

		View newFocused = findFocusableViewInMyBoundsY(movingDown, mScroller
				.getFinalY(), findFocus());
		if (newFocused == null) {
			newFocused = this;
		}
